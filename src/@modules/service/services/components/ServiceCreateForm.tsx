...
import { ICreateService } from "@shared/interfaces";
...
interface IFProps {
  onFinish: (values: ICreateService) => void;
  isLoading?: boolean;
}

const ServiceCreateForm: React.FC<IFProps> = ({ onFinish, isLoading }) => {
  ...
  const handleSubmitForm = (values: ICreateService) => {

    if (imgState.file) {
      values.images = imgState.file;
      onFinish(values)
    } else {
